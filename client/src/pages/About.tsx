          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {specializations.map((spec, idx) => (
              <motion.div 
                ...
                className="flex items-center gap-4 bg-white/10 p-6 rounded-lg backdrop-blur-sm hover:bg-white/20 transition-colors"
              >
            ...
            {businessModelSteps.map((step, index) => (
              <div key={step.title} className="flex items-center">
                <div className="flex flex-col items-center max-w-[200px]">
                  <div className="w-16 h-16 rounded-full bg-secondary text-primary font-bold text-xl flex items-center justify-center mb-4 shadow-lg">
                    {index + 1}
                  </div>
